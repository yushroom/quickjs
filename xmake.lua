...
rule("js")
    ...
    on_load(function (target)
        ...
        target:add("deps", "qjsc", {inherit=false})
    end)
    on_buildcmd_file(function (target, batchcmds, sourcefile_js, opt)
        ...
        local basename = path.basename(sourcefile_js)
        local js_c = path.join(target:autogendir(), "rules", "js_c", basename .. ".c")
        ...
        batchcmds:vrunv(qjsc, {"-c", "-N", "qjsc_"..basename, "-o", js_c, "-m", sourcefile_js})
